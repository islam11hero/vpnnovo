...
export default function AdminError({
  ...
  return (
    <div className="rounded-xl border border-red-500/30 bg-red-950/20 p-10 text-center">
      ...
      <h2 className="font-poppins text-lg font-bold text-white">Command Center fault</h2>
      <p className="mt-2 text-sm text-red-200/80">{error.message}</p>
      <button
        type="button"
        onClick={reset}
        className="mt-6 rounded-lg border border-slate-700 bg-slate-900 px-5 py-2.5 text-sm font-bold text-white"
      >
        Retry
      </button>
    </div>
